    ...
    *resp_drb = *drb;
    ...
    if (f1inst >= 0) { // we actually use F1-U
      ...
      gtpv1u_gnb_create_tunnel_req_t create_tunnel_req = {
        .ue_id = rnti,
        .num_tunnels = 1,
        .outgoing_teid[tunnel_id] = drb->up_ul_tnl[0].teid,
        .outgoing_qfi[tunnel_id][bearer_id].qfi[0] = qfi,
        .pdusession_id[tunnel_id] = drb->drb_id,
        .incoming_rb_id[tunnel_id] = drb->drb_id,
        .dst_addr[tunnel_id].length = 32
      };
      memcpy(&create_tunnel_req.dst_addr[0].buffer, &drb->up_ul_tnl[0].tl_address, sizeof(uint8_t) * 4);
      LOG_D(E1AP, "In %s Incoming RB %ld / Outgoing RB %ld - UL TEID %d QFI %d\n", __func__, drb->drb_id, drb->drb_id, drb->up_ul_tnl[0].teid, qfi);
