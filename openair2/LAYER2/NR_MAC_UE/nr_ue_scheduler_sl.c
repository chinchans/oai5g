...
static void sl_determine_slot_bitmap(uint8_t mod_id)
{
 ...
 */
static uint32_t sl_determine_num_sidelink_slots(uint8_t mod_id, uint16_t *N_SSB_16frames)
{
  NR_UE_MAC_INST_t *mac = get_mac_inst(mod_id);
  sl_nr_ue_mac_params_t *sl_mac = mac->SL_MAC_PARAMS;
...
static uint8_t sl_psbch_scheduler(int module_id, int frame, int slot)
{
 ...
 */
static void sl_adjust_indices_based_on_timing(int mod_id, int frame, int slot, int slots_per_frame)
{
  NR_UE_MAC_INST_t *mac = get_mac_inst(mod_id);
  sl_nr_ue_mac_params_t *sl_mac = mac->SL_MAC_PARAMS;
...
static void sl_actions_after_new_timing(module_id_t mod_id, int frame, int slot)
{
  NR_UE_MAC_INST_t *mac = get_mac_inst(mod_id);
  sl_nr_ue_mac_params_t *sl_mac = mac->SL_MAC_PARAMS;
  ...
  sl_determine_slot_bitmap(mod_id);

  sl_mac->N_SL_SLOTS = sl_determine_num_sidelink_slots(mod_id, &sl_mac->N_SSB_16frames);

  sl_adjust_indices_based_on_timing(mod_id, frame, slot, slots_per_frame);
}

static void sl_schedule_rx_actions(nr_sidelink_indication_t *sl_ind)
{
  NR_UE_MAC_INST_t *mac = get_mac_inst(sl_ind->module_id);
  sl_nr_ue_mac_params_t *sl_mac = mac->SL_MAC_PARAMS;
...
static void sl_schedule_tx_actions(nr_sidelink_indication_t *sl_ind)
{
  NR_UE_MAC_INST_t *mac = get_mac_inst(sl_ind->module_id);
  sl_nr_ue_mac_params_t *sl_mac = mac->SL_MAC_PARAMS;
  ...
  if (tx_action == SL_NR_CONFIG_TYPE_TX_PSBCH) {
    ...
    LOG_I(NR_MAC, "[UE%d] %d:%d CMD to PHY: TX PSBCH \n", sl_ind->module_id, sl_ind->frame_tx, sl_ind->slot_tx);
...
void nr_ue_sidelink_scheduler(nr_sidelink_indication_t *sl_ind)
{
  ...
  if (sl_mac->timing_acquired) {
    sl_actions_after_new_timing(mod_id, sl_ind->frame_tx, sl_ind->slot_tx);
    sl_mac->timing_acquired = 0;
  }
