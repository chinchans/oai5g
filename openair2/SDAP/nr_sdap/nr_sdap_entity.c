...
{
  if (ent != NULL) {
    if (0 != pthread_mutex_lock(&ent->mutex))
...
{
  if (ent != NULL) {
    if (0 != pthread_mutex_unlock(&ent->mutex))
...
bool nr_sdap_delete_entity(ue_id_t ue_id, int pdusession_id, bool deleteIF)
{
  ...
  nr_sdap_lock(sdap_entity); // tun interface read-thread might be using the entity
  SLIST_REMOVE(&entity_head, el, nr_sdap_entity_sl, elements);
  free(el);
  if (deleteIF) {
    remove_ue_ip_if(sdap_entity);
  }
  nr_sdap_unlock(sdap_entity);
  free(sdap_entity);
...
bool nr_sdap_delete_ue_entities(ue_id_t ue_id)
{
  ...
  SLIST_FOREACH(el, &entity_head, elements)
  {
    ...
    if (sdap_entity->ue_id == ue_id) {
      nr_sdap_lock(sdap_entity); // tun IF read thread might be using the entity
      SLIST_REMOVE(&entity_head, el, nr_sdap_entity_sl, elements);
      free(el);
      nr_sdap_unlock(sdap_entity);
      free(sdap_entity);
