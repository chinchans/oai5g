...
static void trigger_bearer_setup(gNB_RRC_INST *rrc, gNB_RRC_UE_t *UE, int nb_pdusessions_tosetup, pdusession_t *sessions, uint64_t ueAggMaxBitRateDownlink)
{
  ...
  for (int i = 0; i < nb_pdusessions_tosetup; i++) {
    ...
    for (int j = 0; j < pdu->numDRB2Setup; j++) {
      ...
      /* Fetch QoS Flow from the PDU Session struct */
      pdusession_level_qos_parameter_t *qos_session = session->qos + j; // TODO: update when implementing multiple QoS per DRB
