...
static int get_ssb_first_scs(const double pointA, const double ssbCenter, const int mu)
{
...
int get_scan_ssb_first_scs(const double fc, const int nbRB, const int nrBand, const int mu, nr_gscn_info_t ssbInfo[MAX_GSCN_BAND])
{
  ...
  for (int g = scanGscnStart; (g <= scanGscnStop) && (numGscn < MAX_GSCN_BAND); g += tmpRaster.step_gscn) {
    ...
    ssbInfo[numGscn].ssbFirstSCS = get_ssb_first_scs(pointA, ssbInfo[numGscn].ssRef, mu);
