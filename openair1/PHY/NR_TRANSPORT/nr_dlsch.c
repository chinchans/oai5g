...
void nr_generate_pdsch(processingData_L1tx_t *msgTx, int frame, int slot)
{
  ...
  for (int dlsch_id=0; dlsch_id<msgTx->num_pdsch_slot; dlsch_id++) {
    ...
    for (int codeWord = 0; codeWord < rel15->NrOfCodewords; codeWord++) {
      ...
      LATSEQ_P("D phy.scrambled--phy.modulated", "len%u::fm%u.sl%u.codeword%u.rnti%u", 1, frame, slot, codeWord, rel15->rnti);
      stop_meas(dlsch_scrambling_stats);
      /// Modulation
      start_meas(dlsch_modulation_stats);
      nr_modulation(scrambled_output, encoded_length, Qm, (int16_t *)mod_symbs[codeWord]);
      LATSEQ_P("D phy.modulated--phy.layermapped", "len%u::fm%u.sl%u.codeword%u.qm%u.rnti%u", 1, frame, slot, codeWord, Qm, rel15->rnti);
