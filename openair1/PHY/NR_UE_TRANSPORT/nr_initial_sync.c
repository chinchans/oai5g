...
void nr_scan_ssb(void *arg)
{
  ...
  for (int nid2 = 0; nid2 < pss_sequence; nid2++)
    generate_pss_nr_time(fp, nid2, ssbInfo->gscnInfo.ssbFirstSCS, pssTime[nid2]);
...
nr_initial_sync_t nr_initial_sync(UE_nr_rxtx_proc_t *proc, PHY_VARS_NR_UE *ue, int n_frames, int sa)
{
  ...
  int numScans = get_scan_ssb_first_scs(fp->dl_CarrierFreq, fp->N_RB_DL, fp->nr_band, fp->numerology_index, gscnInfo);
  notifiedFIFO_t nf;
  initNotifiedFIFO(&nf);
  ...
  fp->ssb_start_subcarrier = res.gscnInfo.ssbFirstSCS;
