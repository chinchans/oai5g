...
void nr_symbol_fep(const PHY_VARS_NR_UE *ue,
                   ...
                   const int link_type,
                   const c16_t *rxdata[ue->frame_parms.nb_antennas_rx],
                   c16_t *rxdataF[ue->frame_parms.nb_antennas_rx])
{
                ...
                uint32_t linktype)
{
  ...
  nr_symbol_fep(ue, Ns, symbol, linktype, rxdata_symb_ptr, rxdataF_symb_ptr);
                          ...
                          int link_type)
{
  ...
  for (unsigned char aa = 0; aa < frame_parms->nb_antennas_rx; aa++) {
    ...
    if (rx_offset + frame_parms->ofdm_symbol_size > frame_length_samples * 2) {
      ...
      memcpy((void *)&tmp_dft_in[aa][0],
             (void *)&common_vars->rxdata[aa][rx_offset],
             (frame_length_samples * 2 - rx_offset) * sizeof(int32_t));
      memcpy((void *)&tmp_dft_in[aa][frame_length_samples * 2 - rx_offset],
  ...
  start_meas(&ue->rx_dft_stats);
  nr_symbol_fep(ue, 0, symbol, link_type, rxdata_symb_ptr, rxdataF_symb_ptr);
  stop_meas(&ue->rx_dft_stats);
